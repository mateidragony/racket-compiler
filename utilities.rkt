...
(struct Function2 (params body env) #:transparent)

(struct StructFunction (method struct-args) #:transparent)
...
(define (compile passes)
  (define prog-file-name (vector-ref (current-command-line-arguments) 0))
  (define file-base (string-trim prog-file-name ".rkt"))
  ((compile-file #f passes) prog-file-name)
  (assemble-file file-base))
...
(define (assemble-file file-name)
  (define gcc-cmd (if (and (equal? (system-type 'os) 'macosx)
                           (equal? (system-type 'arch) 'aarch64))
                      "gcc -g -std=c99 -arch x86_64"
                      "gcc -g -std=c99"))
  (system (format "~a runtime.o ~a.s -o ~a.out" gcc-cmd file-name file-name))
  (void))
