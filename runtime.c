...
#ifdef MATEI_DEBUG
  printf("Initialize:\n");
  printf("\theap_size: %lld\n\tfree_ptr: %p\n", heap_size, free_ptr);
  printf("\tcollection space: %lld\n", sizeof(int64_t) * (fromspace_end - free_ptr));
  printf("\tROOTSTACK:\n\t\trootstack_start: %p\n\t\trootstack_end: %p\n\t\trootstack_size: %lld\n", rootstack_begin, rootstack_end, rootstack_end - rootstack_begin);
  printf("\tFROMSPACE:\n\t\tfromspace_start: %p\n\t\tfromspace_end: %p\n\t\tfromspace_size: %lld\n", fromspace_begin, fromspace_end, fromspace_end - fromspace_begin);
  printf("\tTOSPACE:\n\t\ttospace_start: %p\n\t\ttospace_end: %p\n\t\ttospace_size: %lld\n", tospace_begin, tospace_end, tospace_end - tospace_begin);
...
void print_pointer(int64_t* ptr){
  printf("%p\n", ptr);
}
...
void collect(int64_t** rootstack_ptr, uint64_t bytes_requested)
{
  ...
  printf("Problem child: %d(r15), %p\n", 8 * (10 - (rootstack_ptr - rootstack_begin)), rootstack_begin + 10);
  printf("\ttag ptr: %p\n", *(rootstack_begin + 10));
...
#ifdef MATEI_DEBUG  
  printf("\tPre cheney collection space: %lld\n", sizeof(int64_t) * (fromspace_end - free_ptr));
#endif  
  cheney(rootstack_ptr);
#ifdef MATEI_DEBUG  
  printf("\tPost cheney collection space: %lld\n\n", sizeof(int64_t) * (fromspace_end - free_ptr));
#endif
...
#ifdef MATEI_DEBUG      
    printf("\tPost realloc collection space: %lld\n\n", sizeof(int64_t) * (fromspace_end - free_ptr));
#endif
  }
...
void print_heap(int64_t** rootstack_ptr)
{
  ...
  for (int64_t** root_loc = rootstack_begin;
       ...
       ++root_loc) {
    if (is_ptr(*root_loc)) {
      printf("vector (%p) (%d(r15)): ", root_loc, 8 * (root_loc - rootstack_ptr));
      print_vector(to_ptr(*root_loc));
    } else {
      printf("number (%d(r15)): ", 8 * (root_loc - rootstack_ptr));
