...
(define (dict-append d1 d2)
  (cond
    ((dict-empty? d1) d2)
    (else (define k (car (dict-keys d1)))
          (dict-append (dict-remove d1 k) (dict-set d2 k (dict-ref d1 k))))))
...
(define (reveal-functions-exp env)
  (λ (e)
    (match e
      ...
      [_ (rec-over-exprs e (reveal-functions-exp env))])))
...
(define (limit-functions-exp e)
  (match e
    [(Apply (FunRef f i) es)
     #:when (> i 6)
     (Apply (FunRef f 6) (append (take es 5) (list (Prim 'vector (drop es 5)))))]
    [_ (rec-over-exprs e limit-functions-exp)]))
...
(define (limit-functions-replace-defs env)
  (λ (e)
    (match e
      ...
      [_ (rec-over-exprs e (limit-functions-replace-defs env))])))
...
(define (limit-functions-defs defs)
  (match defs
    ...
    [(cons (Def v ts t i e) defs)
     ...
     (cons (Def v (append (take ts 5) (list `[,tup-name : (Vector ,@(drop (get-fun-arg-tys ts) 5))]))
             t i ((limit-functions-replace-defs (init-limit-env (drop ts 5) tup-name 0)) e))
...
(define (allocate-registers-def v ts t info bls)
  ...
  (Def v ts t (dict-set* (dict-remove info 'locals-types)
                         'root_spilled rs-spilled 'stack_spilled s-spilled 'used_callee callees)
...
(define (p/c-def fun-name info bls final-bls)
  (define sp (dict-ref info 'stack_spilled))
  (define rs (dict-ref info 'root_spilled))
...
(define compiler-passes
  `(
    ("eradicate structs" ,eradicate-structs ,interp-Lfun ,type-check-Lfun)
    ("shrink" ,shrink ,interp-Lfun ,type-check-Lfun)
    ;; ("partial evaluate", pe-Lvec, interp-Lvec, type-check-Lvec)
    ("uniquify" ,uniquify ,interp-Lfun ,type-check-Lfun-has-type)
    ("resolve" ,resolve ,interp-Lfun ,type-check-Lfun)
    ("check bounds" ,check-bounds ,interp-Lfun ,type-check-Lfun)
