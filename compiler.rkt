...
(require "type-check-Llambda.rkt") ;; for typed vars
...
(define (get-fun-args ts) (map (λ (ty) (match ty [`[,x : ,t] x] [_ ty])) ts))
(define (get-fun-arg-tys ts) (map (λ (ty) (match ty [`[,x : ,t] t] [_ '_])) ts))

(define (ls-of l e) (build-list l (λ (x) e)))

(define (type-check-typed-vars type-checker)
  (λ (p) (typed-vars #t) (define t (type-checker p)) (typed-vars #f) t))
  ...
  (lambda (e)
    (match e
      ...
      [(Lambda ts t b)
       ...
       (Lambda (for/list ([arg^ new-args] [t (get-fun-arg-tys ts)])
                 (if (equal? t '_) arg^ `(,arg^ : ,t)))
...
(define (uniquify-arg env args v t)
  (define-values (env^ args^) ((uniquify-args env) args))
  (define new-name (uniquify-name v))
  (values (dict-set env^ v new-name) (if (equal? t '_)
                                         (cons new-name args^)
                                         (cons `[,new-name : ,t] args^))))

(define (uniquify-args env)
  (λ (args)
    (match args
      ['() (values env '())]
      [(cons `[,v : ,t] args) (uniquify-arg env args v t)]
      [(cons v args) (uniquify-arg env args v '_)])))
...
;; check-bounds : L -> L
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (check-vec v i op args len-op)
  (If (Prim '< (list i (Prim len-op (list v))))
      (Prim op args)
      (Prim 'exit '())))
...
(define (check-bounds-exp e)
  (match e
    [(Prim 'vectorof-set! (list v i e2))
     ...
     (check-vec v^ i^ 'vectorof-set! (list v^ i^ (check-bounds-exp e2)) 'vectorof-length)]
    [(Prim 'vectorof-ref (list v i))
     (define v^ (check-bounds-exp v))
     (define i^ (check-bounds-exp i))
     (check-vec v^ i^ 'vectorof-ref (list v^ i^) 'vectorof-length)]
...
;; cast-insert : L -> L
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (prim-inj/proj op es ts inj)
  (Inject (Prim op (for/list ([e es] [t ts])
                     (Project (cast-insert-exp e) t))) inj))
...
(define (cast-insert-exp e)
  (match e
    [(Bool b) (Inject e 'Boolean)]
    [(Int i)  (Inject e 'Integer)]
    [(Void)   (Inject e 'Void)]
    [(Prim 'read '()) (Inject e 'Integer)]
    [(Prim 'print (list e)) (Inject (Prim 'print (list (Project (cast-insert-exp e) 'Integer))) 'Void)]
    [(Prim '+ es) (prim-inj/proj '+ es (ls-of (length es) 'Integer) 'Integer)]
    [(Prim '- es) (prim-inj/proj '- es (ls-of (length es) 'Integer) 'Integer)]
    [(Prim '* es) (prim-inj/proj '* es (ls-of (length es) 'Integer) 'Integer)]
    [(Prim 'eq? (list e1 e2)) (Inject (Prim 'eq? (list (cast-insert-exp e1) (cast-insert-exp e2))) 'Boolean)]
    [(Prim op es) #:when (cmp? op) (prim-inj/proj op es (ls-of (length es) 'Integer) 'Boolean)]
    [(Prim 'not (list e)) (If (Prim 'eq? (list (cast-insert-exp e) (Inject (Bool #f) 'Boolean)))
                              (Inject (Bool #t) 'Boolean) (Inject (Bool #f) 'Boolean))]
    [(If e1 e2 e3) (If (Prim 'eq? (list (cast-insert-exp e1) (Inject (Bool #f) 'Boolean)))
                       (cast-insert-exp e3) (cast-insert-exp e2))]
    [(SetBang v e) (Inject (SetBang v (cast-insert-exp e)) 'Void)]
    [(WhileLoop c b) (Inject (WhileLoop (Project (cast-insert-exp (Prim 'not (list (Prim 'eq? (list c (Bool #f)))))) 'Boolean)
                                        (cast-insert-exp b)) 'Void)]
    [(Prim 'vector es) (Inject (Prim 'vector (map cast-insert-exp es)) (cons 'Vector (ls-of (length es) 'Any)))]
    [(Prim 'vector-length (list e)) (Inject (Prim 'any-vector-length (list (cast-insert-exp e))) 'Integer)]
    [(Prim 'vector-ref (list e1 e2)) (Prim 'any-vector-ref (list (cast-insert-exp e1)
                                                                 (Project (cast-insert-exp e2) 'Integer)))]
    [(Prim 'vector-set! (list e1 e2 e3)) (Inject
                                          (Prim 'any-vector-set! (list (cast-insert-exp e1)
                                                                       (Project (cast-insert-exp e2) 'Integer)
                                                                       (cast-insert-exp e3))) 'Void)]
    [(Apply e es) (Apply (Project (cast-insert-exp e) (append (ls-of (length es) 'Any) '(-> Any)))
                         (map cast-insert-exp es))]
    [(FunRef v arity) (Inject (FunRef v arity) (append (ls-of arity 'Any) '(-> Any)))]
    [(Prim 'procedure-arity (list e)) (Inject (Prim 'procedure-arity (list (cast-insert-exp e))) 'Integer)]
    [(Lambda ts t b) (define ts^ (anyify ts))
                     (Inject (Lambda ts^ t (cast-insert-exp b))
                             (append (get-fun-arg-tys ts^) (list '-> t)))]
    [(Prim op (list e)) #:when (type?? op) (Inject (Prim op (list (cast-insert-exp e))) 'Boolean)]
    [_ (rec-exprs e cast-insert-exp)]))

(define (anyify as)
  (map (λ (a) (match a [`[,x : ,t] a] [_ `[,a : Any]])) as))

(define (cast-insert-def v ts t i e)
  (Def v (anyify ts) t i (if (equal? v 'main) (Project (cast-insert-exp e) 'Integer) (cast-insert-exp e))))

(define (cast-insert p)
  (match-let [((ProgramDefs info defs) p)]
    (ProgramDefs info (rec-defs-new-def defs cast-insert-def))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; unmask-casts : L -> L
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (unmask-project e ftype cont)
  (define tmp (uniquify-name 'cast))
  (Let tmp (unmask-casts-exp e)
       (If (Prim 'eq? (list (Prim 'tag-of-any (list (Var tmp))) (Int (any-tag ftype))))
           (cont (ValueOf (Var tmp) ftype)) (Prim 'exit '()))))

(define (unmask-project-v/f name prim num ty)
  (λ (e)
    (define tmp (uniquify-name name))
    (Let tmp (unmask-casts-exp e)
         (If (Prim 'eq? (list (Prim prim (list (Var tmp))) (Int num)))
             (Var tmp) (Prim 'exit '())))))

(define (unmask-vec op v i [e '()])
  (define v^ (uniquify-name 'vec-cast))
  (define i^ (uniquify-name 'index-cast))
  (Let v^ v (Let i^ i (If (Prim 'eq? (list (Int (any-tag '(Vector))) (Prim 'tag-of-any (list (Var v^)))))
                          (check-vec (Var v^) (Var i^) op (append (list (Var v^) (Var i^)) e) 'any-vector-length)
                          (Prim 'exit '())))))

(define (unmask-casts-exp e)
  (match e
    [(Project e `(Vector . ,tys))
     (unmask-project e `(Vector . ,tys) (unmask-project-v/f 'vec-proj 'any-vector-length (length tys) `(Vector . ,tys)))]
    [(Project e `(,tys ... -> ,rt))
     (unmask-project e `(,@tys -> ,rt) (unmask-project-v/f 'fun-proj 'procedure-arity (length tys) `(,@tys -> ,rt)))]
    [(Project e ftype) (unmask-project e ftype id)]
    [(Inject e ftype) (Prim 'make-any (list (unmask-casts-exp e) (Int (any-tag ftype))))]
    [(Prim op (list e)) #:when (type?? op) (Prim 'eq? (list (Prim 'tag-of-any (list (unmask-casts-exp e)))
                                                            (Int (any-tag (type??->type op)))))]
    [(Prim 'any-vector-ref  (list v i))   (unmask-vec 'any-vector-ref (unmask-casts-exp v) (unmask-casts-exp i))]
    [(Prim 'any-vector-set! (list v i e)) (unmask-vec 'any-vector-set! (unmask-casts-exp v)
                                                      (unmask-casts-exp i) (list (unmask-casts-exp e)))]
    [_ (rec-exprs e unmask-casts-exp)]))

(define (unmask-casts p)
  (match-let [((ProgramDefs info defs) p)]
    (ProgramDefs info (rec-defs defs unmask-casts-exp))))
...
(define (closure-exp escapees)
  (λ (e)    
    (match e
      ...
      [(HasType (Var x) t) (Var x)]
      [(ValueOf e t) (ValueOf ((closure-exp escapees) e) (translate-type t))]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define (optimize-expr env set-bangs)
  (λ (e)
    (match e
      [(Let v (HasType (Closure l (cons (FunRef name a) fvs)) t) e)
       (Let v (Closure l (cons (FunRef name a) fvs)) ((optimize-expr (if (set-member? set-bangs v)
                                                                         env (dict-set env v (cons name a)))
                                                                     set-bangs) e))]
      [(Apply (Prim 'vector-ref (list (Var clos) (Int 0))) es)
       #:when (dict-has-key? env clos)
       (define name/arity (dict-ref env clos))
       (Apply (FunRef (car name/arity) (cdr name/arity)) (for/list ([e es]) ((optimize-expr env set-bangs) e)))]
      [_ (rec-exprs e (optimize-expr env set-bangs))])))

(define (optimize-known-calls p)
  (match-let [((ProgramDefs info defs) p)]
    (ProgramDefs info (rec-defs defs (λ (e) ((optimize-expr '() (collect-set! e)) e))))))
...
(define (vec-clos-ty ty)
  (match ty
    [`(Vector ,`((Vector _) ,tys ... -> ,ty)) (cons tys ty)]
    [else #f]))

(define (limit-functions-exp e)
  (match e
    [(HasType e t) (limit-functions-exp e)]
    ...
    [(ValueOf v t)
     #:when (vec-clos-ty t)
     (define tys (vec-clos-ty t))
     (ValueOf (limit-functions-exp v)
              (if (> (length (car tys)) (sub1 n-arg-regs))
                  `(Vector ((Vector _) ,@(take (car tys) (- n-arg-regs 2))
                                       (Vector ,@(drop (car tys) (- n-arg-regs 2))) -> ,(cdr tys)))
                  `(Vector ((Vector _) ,@(car tys) -> ,(cdr tys)))))]
...
(define (create-array len ini type alloc-label)
  (define arr-len (uniquify-name 'arr-len))
  (Let arr-len len       
       (Let '_ (collect-check (Prim '* (list (Int 8) (Prim '+ (list (Int 1) (Var arr-len))))))
            (Let alloc-label (AllocateArray (Var arr-len) type)
                 (vector-set-array (uniquify-name 'idx) (Var arr-len) ini alloc-label)))))
...
(define (rco-exp e)
  (match e
    ...
    [(ValueOf e t) (rco-atom e (λ (v) (ValueOf v t)))]
...
(define (explicate-pred p bt bf)
  (match p
    [(Var x) (pred-eq? (Var x) (Bool #t) bt bf)]
    [(GetBang x) (pred-eq? (Var x) (Bool #t) bt bf)]
    ...
    [(Prim 'not (list e)) (pred-eq? e (Bool #f) bt bf)]
    ...
    [(ValueOf e t) (pred-assign 'pred-valof p bt bf)]))

(define (pred-eq? e v bt bf)
  (IfStmt (Prim 'eq? (list e v)) (force (create-block bt)) (force (create-block bf))))

(define (pred-assign name p bt bf)
  (define tmp (uniquify-name name))
  (explicate-assign tmp p (pred-eq? (Var tmp) (Bool #t) bt bf)))
...
(define (explicate-effect e cont [se #f])
  ...
  (if (not (car side-effects)) ;; no side effects anywhere
      ...
      (match e
        ...
        [(Prim 'any-vector-set! (list e1 e2 e3)) (Seq (Prim 'any-vector-set! (list e1 e2 e3)) (force cont))]
  ...
  (match e
    ...
    [(Prim 'any-vector-length (list e))
     (list (Instr 'movq (list (Imm -8) (Reg 'r11)))
           (Instr 'andq (list (select-atm e) (Reg 'r11)))
    ...
    [(Prim 'any-vector-ref (list e1 e2))
     (list (Instr 'movq  (list (Imm -8) (Reg 'r11)))
           (Instr 'andq  (list (select-atm e1) (Reg 'r11)))
           (Instr 'movq  (list (select-atm e2) (Reg 'rax)))
           (Instr 'addq  (list (Imm 1) (Reg 'rax)))
           (Instr 'imulq (list (Imm 8) (Reg 'rax)))
           (Instr 'addq  (list (Reg 'rax) (Reg 'r11)))
           (Instr 'movq  (list (Deref 'r11 0) x)))]
    [(Prim 'any-vector-set! (list e1 e2 e3))
     (list (Instr 'movq  (list (Imm -8) (Reg 'r11)))
           ...
           (Instr 'imulq (list (Imm 8) (Reg 'rax)))
           (Instr 'addq  (list (Reg 'rax) (Reg 'r11)))
           (Instr 'movq  (list (select-atm e3) (Deref 'r11 0)))
           (Instr 'movq  (list (Imm 0) x)))]
    [(Prim 'procedure-arity (list proc))
     (list (Instr 'movq (list (select-atm proc) (Reg 'r11)))
           (Instr 'movq (list (Deref 'r11 0) x))
           (Instr 'sarq (list (Imm 57) x)))]
    [(Prim 'make-any (list e (Int t)))
     #:when (or (equal? t (any-tag '(Vector))) (equal? t (any-tag '(-> Any))))
     (list (Instr 'movq (list (select-atm e) x))
           (Instr 'orq  (list (Imm t) x)))]
    [(Prim 'make-any (list e (Int t)))
     (list (Instr 'movq (list (select-atm e) x))
    ...
    [(ValueOf e ty)
     #:when (or (equal? ty 'Boolean) (equal? ty 'Integer))
     (list (Instr 'movq (list (select-atm e) x))
           (Instr 'sarq (list (Imm 3) x)))]
    [(ValueOf e ty)
     (list (Instr 'movq (list (Imm -8) x))
           (Instr 'andq (list (select-atm e) x)))]
    [(Allocate n type)
...
(define (propogate-instr instr env) ;; instr, new env
  (match instr
    ...
    [(Instr 'orq   (list a1 a2)) (propogate-tuah a1 a2 'orq   bitwise-ior env instr #t)]
...
(define (is-terminal instrs)
  (match instrs
    ['() #f]
    [(cons (Callq 'exit 1) _) #t]
    [(cons i instrs) (is-terminal instrs)]))

(define (terminal-labels fun-name bls)
  (cond
    ((dict-empty? bls) (set (symbol-append fun-name '_conclusion)))
    (else (define label (car (dict-keys bls)))
          (match-let [((Block info instrs) (dict-ref bls label))]
            (if (is-terminal instrs)
                (set-add (terminal-labels fun-name (dict-remove bls label)) label)
                (terminal-labels fun-name (dict-remove bls label)))))))

(define (cp-transfer bls fun-name terminals)
  (λ (label pre-env)
    (if (set-member? terminals label)
        pre-env
        (match-let [((Block info instrs) (dict-ref bls label))]
  ...
  (cond
    ...
    (else (define label (car (dict-keys bls)))
          (match-let ([(Block info instrs) (dict-ref bls label)])
            ...
            (for ([jmp jmp-labels])
              (add-directed-edge! cfg label jmp)
              (add-directed-edge! cfg-only-jmp label jmp))
            (for ([jmp jmpif-labels])
              (add-directed-edge! cfg label jmp))
...
(define (constant-propogation-def fun-name ts t info lts)
  (define terminals (terminal-labels fun-name lts))
  (define cfg (make-multigraph '()))
  (define cfg-only-jmp (make-multigraph '()))
  (build-cf-graph cfg cfg-only-jmp lts)
  (for ([t terminals]) (add-vertex! cfg t))
  (define df-analysis (analyze_dataflow cfg (cp-transfer lts fun-name terminals) (make-immutable-hash) cp-union))
...
(define (add-edge instr live-after locals-types ig mg)
  ...
  (when (call? instr)
    (for* [(d callee-save)
           (v (filter (λ (e) (or (type-vector? (dict-ref locals-types e 'Other))
                                 (equal? 'Any  (dict-ref locals-types e 'Other))))
                      (set->list live-after)))]
      (add-edge! ig d v))))
  ...
  (if (<= c 10)
      (let ((r (color->reg c)))
        (values (Reg r) smap rsmap (if (set-member? callee-save r)
                                       ...
                                       callees)))
      (if (or (type-vector? t) (equal? 'Any t))
...
(define compiler-passes
  `(
    ("eradicate structs" ,eradicate-structs ,interp-Ldyn)
    ("shrink" ,shrink ,interp-Ldyn-prog)
    ("uniquify" ,uniquify ,interp-Ldyn-prog)
    ("reveal functions" ,reveal-functions ,interp-Ldyn-prog)
    ("cast insert" ,cast-insert ,interp-Lany-prime ,type-check-Lany)
    ("unmask casts" ,unmask-casts ,interp-Lany-prime ,(type-check-typed-vars type-check-Lany))
    ("convert assignments" ,convert-assignments ,interp-Lany-prime ,(type-check-typed-vars type-check-Lany))
    ("closure conversion" ,closure-conversion ,interp-Lany-prime ,type-check-Lany)
    ("optimize known calls" ,optimize-known-calls ,interp-Lany-prime ,type-check-Lany)
    ("resolve" ,resolve ,interp-Lany-prime ,type-check-Lany)
    ("check bounds" ,check-bounds ,interp-Lany-prime ,type-check-Lany)
    ("limit functions" ,limit-functions ,interp-Lany-prime ,type-check-Lany-has-type)
    ...
    ("remove complex opera*" ,remove-complex-opera* ,interp-Lany-prime ,type-check-Lany)
