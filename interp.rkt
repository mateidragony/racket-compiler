    ...
    (define/public (interp-x86-instr env)
      (lambda (ast)
	   ...
	   [(cons (Callq 'read_int _) ss)
           ...
           [(cons (Callq 'print_any _) ss)
            (let ([v (lookup 'rdi env)])
              (copious "print " v)
              ((interp-x86-instr (cons (cons 'rax 0) env)) ss))]
...
(define interp-R4-class
  (class interp-R3-class
    ...
    (define/public (builtin-funs)
      (set 'malloc 'alloc 'collect 'initialize 'read_int 'exit 'print_any 'print_int))
